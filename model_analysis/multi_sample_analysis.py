...
def analyze_and_save_results(df, X_test, y_test, folder_path, weights, output_file, target_name, target_col_name, feature_ranges):
    sample_indices = range(0,X_test.shape[0])
...
small_model_feature_ranges = {
    ...
    'Fraction of prismatic phase':np.arange(0,20.5,0.5),
    'Fraction of basal phase': np.arange(0,20.5,0.5)
    
}

# 分析并保存屈服强度和抗拉强度的结果（小模型）
# analyze_and_save_results(df_qf, X_test_qf, y_test_qf, folder_path_qf, weights_qf, 'AAA_small_qf_multiple_single_variable_analysis.xlsx', 'Yield Strength', 'Yield_Strength', small_model_feature_ranges)
analyze_and_save_results(df_kl, X_test_kl, y_test_kl, folder_path_kl, weights_kl, 'AAA_small_kl_multiple_single_variable_analysis.xlsx', 'Tensile Strength', 'Tensile_Strength （UTS）', small_model_feature_ranges)
