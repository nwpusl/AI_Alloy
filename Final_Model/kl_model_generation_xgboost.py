...
label='kl'
main_path=f'models/{label}'  #  研究抗拉强度
model_name1 = 'xgboost'
...
# 删除包含空值的行
if del_0qf:
    df = df[df['Tensile_Strength （UTS）'] != 0].reset_index(drop=True)
feature_names = df.columns[:-4]  # 特征名为屈服强度（倒数第四列）之前的部分
X = df.iloc[:, :-4]  # 特征: 最后四列之前的所有列
y = df['Tensile_Strength （UTS）']  # 目标: 倒数第四列
# print('X.columns:\n',X.columns)
# print('特征的形状:\n',len(feature_names))
# print('输入数据的形状：\n',X.shape)
...
write_to_existing_excel(model_scores_df, save_path, 'kl_xgboost')